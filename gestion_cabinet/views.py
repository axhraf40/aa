...
def home(request):
    ...
    except InformationCabinet.DoesNotExist:
        info_cabinet = None
    
    # Récupérer l'équipe
    equipe = MembreEquipe.objects.all()
    
    # Récupérer les spécialités avec débogage
    specialites = SpecialiteCabinet.objects.all()
    print("Nombre de spécialités trouvées:", specialites.count())
    for spec in specialites:
        print(f"- {spec.nom} (image: {spec.image})")
    
    context = {
        'info_cabinet': info_cabinet,
        'equipe': equipe,
        'specialites': specialites,
        'adresse': info_cabinet.adresse if info_cabinet else "",
        'telephone': info_cabinet.telephone if info_cabinet else "",
        'email': info_cabinet.email if info_cabinet else "",
        'google_maps_url': info_cabinet.google_maps_url if info_cabinet else "",
    }
    
    return render(request, 'gestion_cabinet/home.html', context)
